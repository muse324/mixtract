...
public class Util {
	...
	public static final int REST_NOTE = -1;
	 ...
	 * @return
	 */
	public static double log2percent(double log) {
		return Math.pow(2., log) * 100.;
	}
	...
	/**
	 * 「何もしない」ことを明示的に示すための空メソッドです． 実装し忘れではないということをはっきりさせておきたい場合などに用います．
	 * 「何も実行しないがこの行まで処理が来たことを確認したい」というような場合の
	 * デバッグプリントなどを行いたい場合はこのメソッドをオーバーライドしてください．
	 */
	...
	public static final int valueOfFifths(String key) {
		return KeyFifths.valueOf(key).fifths();
	}
	...
	/**
	 * 入力ファイル名から拡張子を除いた文字列を得る。
	 *
	 * @param s
	 * @return String
	 */
	public static String getNameExceptExtension(String strName) {
		return strName.replace(SEPARATOR + getExtension(strName), "");
	}
	...
	public static int getNoteNumber(String noteName) {
		...
		String oct = noteName.substring(noteName.length() - 1);
		return (Integer.parseInt(oct) + 1) * 12 + (pitch + acci);
	}

	public static int getNoteScale(char noteChar) {
		int n = 0;
		switch (noteChar) {
			case 'C':
				n = 0;
